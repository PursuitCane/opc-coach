// 优化商业计划：先由 AI 找出 4 个信息缺口 → 用户答完 → 合成 v2 计划书
...
要求：恰好 4 道题。问题要针对当前项目最缺、材料里读不到的信息。避免"你的目标客户是谁"这种大而空的题。`
...
现在你要为这个项目产出一份完整的商业计划书。在动笔前，请找出 4 个「材料里读不到、又绕不过去」的信息缺口，把它们变成 4 道给用户的具体问题。
...
${QUESTIONS_SHAPE}`
  ...
  return result.questions.slice(0, 4)
}
...
- 每个 section 的 markdown 至少 3 段，要引用用户在 4 道题里给出的答案（如果哪题空着就把该章 flag 标 "待补充" 并在正文里说明缺什么）
