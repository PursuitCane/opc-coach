...
export function Login() {
  ...
  return <div className="landing">
    ...
    <section className="landing-hero"><div><h1>OPC<br />COACH</h1><div className="landing-rule" /><p className="landing-lead">你做项目时是否遇到这些问题：</p><div className="landing-pains">{PAINS.map(([title, body]) => <div className="landing-pain" key={title}><i /><div><strong>{title}</strong><span>{body}</span></div></div>)}</div><div className="landing-divider" /><div className="landing-help"><h2>我们帮你梳理！</h2><p>陪你把想法变成<br />可验证、可增长、可落地的项目</p></div></div>
      <div className="landing-signin" id="signin"><h3>开始使用</h3><p>上传一份材料，几分钟内拿到第一版商业分析。</p><div className="field"><label>邮箱</label><input className="input" defaultValue="lin@onepersonco.cn" /></div><div className="field"><label>验证码</label><div className="landing-code"><input className="input" defaultValue="· · · ·" /><button className="btn btn-secondary">获取验证码</button></div></div><button className="btn btn-primary btn-block" style={{ height: 40 }} onClick={doLogin}>登录，进入工作台</button><small>登录即表示同意服务条款与隐私政策</small></div>
    </section>
